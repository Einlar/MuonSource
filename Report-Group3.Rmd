...
plt.scatter(deltay , momenta[:,1]/momenta[:,0], s = .5)
```
...
```{python}
gamma_mus = np.sqrt(pnorms_sq + mmu**2) / mmu
```
...
```{python}
y0, z0 = generate_yz(num_samples, 1) #y0, z0 in [cm]
y0 /= 100 #to [m]
z0 /= 100
```
...
#                            - np.square(vy) ) - vx ) / omegas

yf=np.sqrt(np.square(radii)- np.square(vy/omegas-l_magnet)) -vx/omegas+y0
```
...
Since the relation between momentum and velocity is $p_i=\gamma mv_i$  we have
$$
...
**Edit** Last equation should be:
$$\omega t = \alpha + \arcsin \left( \frac{\omega( x(\hat{t}) - x_0 )}{|v_{\rm pe}|} \right) $$
This works if $x(\hat{t})$ is reachable (this is indeed the case).
Also one needs to subtract the center coordinates from $x(t)$ and $y(t)$ (in the solution, I think, the origin is set as the center)
...
```{python}
#Fixed the c
def bend_particle (position, momentum, meters): #to be vectorized
    """Computes the final position and momentum of a particle starting at @position, with @momentum, subject to a constant magnetic field
    ...
    Returns
    -------
    final_pos, final_p : Tuples of ndarray, each of dimension (3,)
       final_pos = Final coordinates for the particle
       final_p   = Final particle momentum
    """
    
    p     = np.linalg.norm(momentum) 
    beta  = p / np.sqrt(p**2 + mmu**2)
    gamma = 1 / math.sqrt(1 - beta**2)
    omega = (B * c**2) / (mmu * 1e6 * gamma)
    
    #print(p, beta, gamma)
    
    alpha = -math.atan(momentum[1] / momentum[0]) #fase nel sen e coseno
    #print(alpha)
    v_w   = np.sqrt(momentum[0]**2 + momentum[1]**2) * (1e6) / (B * c) #rapporto v perperdicolare su omega, il 10^6 è perche il momento è in Mev
    ...
    wt = alpha + np.arcsin((meters - position[0]) / v_w) #omega per tempo (EDITED)
    #print(wt)
    
    final_position = np.array([v_w * np.sin(wt - alpha), v_w * np.cos(wt - alpha), momentum[2] * wt * 1e6 / (B * c)])
    final_position += np.array(position) #sum initial position
    final_position += np.array([momentum[1] * 1e6 / (B * c), -momentum[0] * 1e6 / (B * c), 0]) #Add center coordinates
    
    p_pe = np.sqrt(momentum[0]**2 + momentum[1]**2)
    final_momentum = np.array([p_pe * np.cos(wt - alpha), p_pe * np.sin(wt - alpha), momentum[2]])
...
def free_particle(position, momentum, meters):  #to be reviewed + vectorized
    p     = np.linalg.norm(momentum) 
    beta  = p / np.sqrt(p**2 + mmu**2)
    gamma = 1 / math.sqrt(1 - beta**2)
    velox= momentum*1e8/(mmu*gamma)
    final_position=[]
    time=meters/velox[0] #time travel on the x axis
    final_position.append(position[0]+meters) #final x position
    final_position.append(position[1]+velox[1]*time) #final y position
    final_position.append(position[2]+velox[2]*time) #final z position
    return final_position, momentum
```
...
bend_particle(positions[0] / 100, momenta[0,:3], 2)
```
...
for i in range(num_samples):
    f_pos, f_mom = bend_particle(positions[i] / 100, momenta[i, :3], 2)
    f_x.append(f_pos[0])
    f_y.append(f_pos[1])
    f_z.append(f_pos[2])
    f_px.append(f_mom[0])
...
plt.xlabel(r"$\Delta y$ [cm]")
plt.ylabel(r"$\Delta z$ [cm]")
plt.scatter(positions[:,1], positions[:,2], s=.9, c='red', label="Before magnet")
plt.scatter((f_y-np.mean(f_y))*100, (f_z-np.mean(f_z))*100, s=.9, label="After magnet", c='b') #in [cm]
plt.legend()
plt.plot()

#quite different from the other one. Why?
```

```{python}
bended_positions = np.stack((f_x, f_y, f_z), axis=1)
bended_momenta = np.stack((f_px, f_py, f_pz), axis=1)


b_y = []
b_z = []
for i in range(num_samples):
    f_pos, f_mom = free_particle(bended_positions[i], bended_momenta[i], 4)
    b_y.append(f_pos[1])
    b_z.append(f_pos[2])


#Plot after 4 meter of free propagation after magnet
plt.xlabel(r"$\Delta y$ [cm]")
plt.ylabel(r"$\Delta z$ [cm]")
plt.scatter((f_y-np.mean(f_y))*100, (f_z-np.mean(f_z))*100, s=.9, label="After magnet", c='b') #in [cm]
plt.scatter((b_y-np.mean(b_y))*100, (b_z-np.mean(b_z))*100, s=.9, label="After propagation", c='g') #in [cm]
plt.legend()
plt.plot()
...
```{python}
plt.figure(figsize=(6,6))
plt.hist2d(positions[:,1], positions[:,2],bins=100)
plt.xlabel('y')
plt.ylabel('z')
plt.show()

plt.figure(figsize=(6,6))
plt.hist2d((f_y-np.mean(f_y))*100, (f_z-np.mean(f_z))*100,bins=100)
plt.xlabel('y')
plt.ylabel('z')
plt.show()

plt.figure(figsize=(6,6))
plt.hist2d((b_y-np.mean(b_y))*100, (b_z-np.mean(f_z))*100,bins=100)
plt.xlabel('y')
plt.ylabel('z')
plt.show()
