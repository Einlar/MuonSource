...
#Momentum boost
E_cm = sqrs / 2
E_mu_lab1  = gamma_e(s) * (E_cm  - px_cm*beta_e(s))
...
rho_electrons = NA * rhoBe * ZBe / MBe
...
```{python}
def generate(N, r, mu, sigma, batch_size=10000):
    """Generates (y,z) and E according to the g and h distributions (4.1). Uses rejection sampling for (y,z)
    ...
    mu, sigma : float
       Parameters for the gaussian energy distribution
    batch_size : int, optional
       Random sampling is done in batches of this size, to increase performance
       
    Returns
    -------
    y, z, E : tuple of ndarrays
       Sampled values for y, z and E
    """
    
    generated = 0 
    ys = np.zeros(N, dtype=float)
    zs = np.zeros(N, dtype=float)
    Es = np.random.normal(mu, sigma, size=N)
...
```{python}
ys, zs, Es = generate(1000000, 1, 52.9, 0.5) #without batches it would take 17s
```
...
plt.plot(thetas,g_cdf(230**2,thetas))
...
```{python}
k = 1.0
def event_generator(N, r=1, E=52.9, sigma=0.5): #to be vectorized
